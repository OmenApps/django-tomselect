...
class TomSelectField(forms.ModelChoiceField):
    ...
    def __init__(self, *args, queryset=EmptyModel.objects.none(), config: TomSelectConfig = None, **kwargs):
        ...
        self.config = config or TomSelectConfig(**kwargs)
        ...
        field_kwargs = {k: v for k, v in kwargs.items() if not hasattr(self.config, k)}
        super().__init__(queryset, *args, **field_kwargs)
